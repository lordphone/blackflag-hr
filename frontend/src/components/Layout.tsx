...
export default function Layout({ children }: LayoutProps) {
  ...
  const { user, logout, notifications, clearNotification, getUnreadCount } = useApp()
          ...
          {navigation.map((item) => (
            ...
            >
              ...
              {item.name === 'Messages' && unreadMessages > 0 && (
                <span className="ml-auto w-5 h-5 bg-stone-900 text-white text-xs font-medium rounded-full flex items-center justify-center">
                  {unreadMessages}
                </span>
              )}
            </Link>
